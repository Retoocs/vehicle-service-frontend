...
const localAllowedNetsFactory = (factory: AllowedNetsServiceFactory) => {
  return factory.createFromArray(['customer']);
};

const baseFilterFactory = () => {
  return {
    filter: SimpleFilter.fromCaseQuery({
      process: {identifier: 'customer'},
...
@Component({
  selector: 'app-side-nav-customers-content1-case-view',
  templateUrl: './side-nav-customers-content1-case-view.component.html',
  styleUrls: ['./side-nav-customers-content1-case-view.component.scss'],
  providers: [
    CategoryFactory,
    CaseViewService,
    SearchService,
...
export class SideNavCustomersContent1CaseViewComponent extends TabbedCaseView implements AfterViewInit {
  ...
  createNewCase(): Observable<Case> {
    const myCase = this.createInstance('customer');
