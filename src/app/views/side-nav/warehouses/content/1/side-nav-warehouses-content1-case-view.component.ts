...
const localAllowedNetsFactory = (factory: AllowedNetsServiceFactory) => {
  return factory.createFromArray(['warehouse']);
};

const baseFilterFactory = () => {
  return {
    filter: SimpleFilter.fromCaseQuery({
      process: {identifier: 'warehouse'},
    })
  };
};

@Component({
  selector: 'app-side-nav-warehouses-content1-case-view',
  templateUrl: './side-nav-warehouses-content1-case-view.component.html',
  styleUrls: ['./side-nav-warehouses-content1-case-view.component.scss'],
  providers: [
    CategoryFactory,
    CaseViewService,
    SearchService,
...
export class SideNavWarehousesContent1CaseViewComponent extends TabbedCaseView implements AfterViewInit {
  ...
  createNewCase(): Observable<Case> {
    const myCase = this.createInstance('warehouse');
